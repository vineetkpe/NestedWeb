...
The follow-up adds regressions for unknown/empty/inherited failure-code names and first-person description placeholders, plus coverage of every declared crawler failure, product/description conflicts with exact source references, and instruction text embedded inside an eligible claim. The focused command is `node --conditions=react-server --test src/application/company-profile.test.ts`. Interpretation now reports `company-profile-v2`; the original implementation's verification record remains historical in its task plan.

Prompt-generation tests use the real extractor with test-only source fixtures. Run `node --test src/application/prompt-generation.test.ts`; the existing unit glob includes the suite. Coverage includes all six query categories, exact questions, sparse inputs and each missing prerequisite, separate product/company conflicts, category/audience conflicts, duplicate and multiple use cases, case/whitespace/hyphen normalization without merging C++ and C#, equivalent questions across templates, qualified service-area evidence, inert instructions, repeatable ordering/IDs, unchanged input, and resolution of every reference through the profile to the original source span. Boundary tests cover malformed version/fields/cardinality/spans/URLs, excluded or inconsistent pages, sparse evidence arrays, malformed Unicode, and excessive provenance. Large eligible profiles produce at most ten questions; overlong terms are omitted without truncation. No live provider or database is involved.

Database integration suites exist under `supabase/tests`, covering tenancy and subsequent scan/evidence boundaries. `npm run test:db` runs pgTAP against local Supabase separately from `npm run verify`; passing application checks does not prove RLS or migration correctness. `.github/workflows/ci.yml` includes a disposable database job that starts Supabase, replays migrations, and runs those suites. [The security exit record](docs/security-exit.md) records historical hosted checks; neither that record nor configured CI proves the current checkout passed.

Use a disposable local database, never customer data, for fixtures and migration replay. Record actual command results and database availability for each task. Tests use actual anonymous/authenticated roles and rollback fixtures; service-role success alone is not an isolation check.

## Grounded provider tests

The Gemini boundary has a focused suite at `src/infrastructure/gemini.test.ts`, run with `node --conditions=react-server --test src/infrastructure/gemini.test.ts`. It uses synthetic GenerateContent bodies and injected native `Response` objects, never real credentials or network transport. A captured test execution-clock value verifies timestamp provenance; no fabricated provider capture date is asserted. Tests cover exact generated-query identity, unchanged HTTP bodies and stable SHA-256 digests, structured-only citations and raw grounding metadata, duplicate/unsafe URLs, refusal/partial/failure, missing models, malformed responses, size/citation bounds, cancellation/deadlines/busy state, credential echoes including JSON escapes, closed live execution, inert instructions, and default-condition rejection of the server-only adapter. Existing Firecrawl tests exercise its unchanged behavior after sharing the bounded body reader. This does not verify real provider behavior or remove the unresolved database checks above.

## Single-scan tests

Run `node --conditions=react-server --test src/application/scan.test.ts`. The suite composes actual CompanyProfile extraction and prompt generation before invoking the runner with a deterministic fake implementing the shared provider interface; it imports no live adapter. Tests assert exact call order/count, one active call, ten-query and sparse cohorts, unique stable attempt IDs, exact query/version preservation, answered/refused/partial/failed entries, successful evidence retained across provider failures, no retries, cancellation before/between/during calls, provider-not-executed handling, malformed/oversized/duplicate/version-inconsistent cohorts, invalid provider configurations, observation identity/size checks, snapshots across caller/provider mutation, and inert malicious text. Scan identity alphabet and hidden-accessor regressions were observed failing before correction. No scanner timestamps are fabricated; observation times come from the fake provider's captured test execution clock. Shared profile-validation and prompt-ID refactors retain the existing extraction/prompt/provider regression suites.
