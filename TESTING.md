...
A draft database integration suite exists at `supabase/tests/core_tenancy_test.sql`; it has not executed successfully because the local Supabase database is unavailable. This is an unresolved check, not a passing suite. Domain intake, company-profile extraction, and prompt generation have no Supabase dependency.
