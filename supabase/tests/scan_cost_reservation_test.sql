...
select is(
  (select count(*) from app_private.scan_provider_configs),
  0::bigint,
  'production migrations seed no provider pricing'
);
select is(
  (select count(*) from app_private.workspace_scan_controls),
  0::bigint,
  'production migrations seed no workspace paid-call entitlement'
);
select is(
  (select count(*) from app_private.project_scan_controls),
  0::bigint,
...
select ok(
  not has_table_privilege('authenticated', 'app_private.scan_provider_configs', 'select'),
  'authenticated callers cannot read provider pricing controls'
);
select ok(
  not has_table_privilege('authenticated', 'app_private.scan_cost_reservations', 'select'),
  'authenticated callers cannot read private cost ledger rows'
);
select ok(
  has_function_privilege(
    'authenticated',
    'public.reserve_scan(uuid,uuid,uuid,text,text,jsonb)',
    'execute'
  ),
...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.settle_scan_reservation(uuid,uuid,uuid,bigint)',
    'execute'
  ),
...
select is(
  ...
  '400',
  'worst-case reservation is queries times allowed attempts times configured upper bound'
);
...
select throws_ok(
  $$select * from app_private.scan_cost_reservations$$,
  '42501',
  null,
...
select is(
  ...
  '200',
  'a second request reserves the remaining project budget'
);
select throws_ok(
  $$select public.reserve_scan(
    'c2000000-0000-4000-8000-000000000001',
...
select is(
  ...
  'settled',
  'service settlement records confirmed cost separately from reserved cost'
);
select is(
  (
    public.settle_scan_reservation(
  ...
  'true',
  'same settlement key and amount is idempotent'
);
select throws_ok(
  ...
  '22023',
  'Settlement key reused with different scan settlement',
...
select is(
  (
    public.reserve_scan(
      ...
      'niche-prompts-v1',
      'company-profile-v2',
      '[{"queryId":"niche-prompts-v1:test-5","queryVersion":"category@v1","queryText":"After release"}]'::jsonb
    ) ->> 'reservedMicrounits'
  ),
  '200',
...
select set_config(
  'request.jwt.claims',
  '{"sub":"c1000000-0000-4000-8000-000000000003","role":"authenticated","user_metadata":{"workspace_id":"c2000000-0000-4000-8000-000000000001","role":"owner"}}',
  true
);
