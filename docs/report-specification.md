...
ADR-012 now provides a standalone `ScanResult` containing the supplied profile/cohort and one ordered `ScanAttempt` per planned prompt. The runner accepts 1–10 prompts without regenerating them. Attempt states distinguish answered, refused, partial, failed, cancelled and not attempted. `providerCalled` records boundary invocation, which is distinct from a provider reporting actual execution; unattempted entries have no observation. Reserved attempt IDs derive from caller scan/attempt identities and position. Cancellation retains already returned observations and stops later calls; one failed query does not erase other evidence. `settled` means the in-process loop finished, not that every query answered or the report is ready. No persistence or report route consumes this result.
