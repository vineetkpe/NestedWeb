...
## ADR-011: one grounded provider boundary, live execution closed

Status: authorized by the user's provider-boundary request on 2026-09-10. Level 2 preparation ahead of unfinished Level 1 authorization/database and atomic usage/cost reservation. This exception implements only a single-query contract and mocked adapter; scanner, jobs, persistence, interpretation, UI, and paid execution remain deferred. Stop after review and the four requested checks; no commit.

No earlier ADR selects a provider. Choose Gemini with Google Search through the documented [GenerateContent REST contract](https://ai.google.dev/api/generate-content) and [GenerateContent grounding metadata](https://ai.google.dev/gemini-api/docs/generate-content/google-search). This is the provider's legacy GenerateContent surface, not its newer Interactions surface. One native adapter is sufficient; no SDK, provider registry, or fallback. A requested model is explicit server configuration, never a claim about the response's model version. Model availability and deployment settings must be reviewed before enabling live calls.

Domain observation/citation types and an application provider interface preserve query identity/version/text, exact UTF-8 response body and SHA-256 digest, optional reported model/response IDs, supplied execution-clock time, answer text, and separate grounding metadata. Observation IDs are caller-supplied attempt identities, never timestamps. Normalization only captures provider fields; it performs no mention, sentiment, recommendation, competitor, or metric interpretation.

Only `candidates[0].groundingMetadata.groundingChunks[*].web` creates citation records. Preserve occurrence indexes and duplicate URLs, source titles, and the complete grounding metadata separately; relationships remain `source_list_only` without interpreting support spans. Invalid URLs remain inert records with an exclusion reason and no source domain. Never resolve redirect URLs, inspect answer text for links, infer company citations, or fetch evidence. Preserve raw provider metadata even when normalization fails, subject to size and credential-echo rejection.
...
Execute 1 to 10 supplied queries sequentially, once each, in caller order. Validate the complete input before calling the provider: bounded scan/attempt IDs, existing profile schema, cohort/profile method versions, planned states, unique query IDs, template/category compatibility, versioned query identity and resolvable profile evidence indexes. Share the existing profile validator and query-identity function; do not re-extract, reword, merge or regenerate anything. Snapshot bounded input before awaiting provider work.

Accept the existing GroundedAIProvider interface as an injected trusted dependency. Relax only its provider/grounding labels and the observation capture-version label to bounded strings so orchestration need not know Gemini. The real Gemini adapter and its closed live transport remain unchanged. Provider capability validation requires liveExecution false and single-query bounded operation; this is a fail-closed configuration check, not authorization or proof that arbitrary injected code is harmless. The provider owns its execution clock and deadline; the runner creates no timestamps and waits for an active call to settle after cancellation.
