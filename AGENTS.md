...
Next.js App Router / React / strict TypeScript / Tailwind app using npm and Node 24. Routes `/` and `/report` remain static previews. Server-side auth/tenancy, projects, native entry-page crawling, profile/prompt generation, durable scans/cost reservations, explicit live Gemini transport, evidence persistence, citation normalization, aliases, and mention detection exist. Recommendation classification is a pure core; metrics, customer action recommendations, connected customer UI, billing, and production delivery remain unfinished. See [project progress](docs/project-progress.md) and the current section of [ARCHITECTURE.md](ARCHITECTURE.md); historical task descriptions are not current implementation inventories. Database migrations/tests and database CI exist; historical hosted checks do not prove current production readiness. Do not describe source code or plans as deployed features. `.vscode/mcp.json` predates this work; preserve it and keep credentials out of output.
