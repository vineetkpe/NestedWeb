...
test("configured live worker rejects missing and malformed secrets before claiming work", async () => {
  ...
  > = [
    ...
    [{ ...validBase, SUPABASE_URL: "http://example.com" }, "invalid_supabase_url"],
    [
    ...
    [{ ...validBase, NESTEDWEB_SCAN_WORKER_ID: "not-a-uuid" }, "invalid_worker_id"],
    [{ ...validBase, NESTEDWEB_SCAN_LEASE_SECONDS: "20" }, "invalid_lease_seconds"],
  ];
