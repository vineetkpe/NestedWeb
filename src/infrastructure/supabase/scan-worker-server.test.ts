...
test(
  "configured live worker is disabled by default before any backend client is created",
  async () => {
...
test(
  "configured live worker rejects missing and malformed secrets before claiming work",
  async () => {
    const cases: Array<readonly [Record<string, unknown>, string]> = [
      ...
      [
        { ...validBase, GEMINI_API_KEY: "short" },
